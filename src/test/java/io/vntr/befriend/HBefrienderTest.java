...
public class HBefrienderTest {
    ...
    public void testCalcEdgeCutMove() {
        ...
        partitionsToExpectedCut.put(initSet(1, 2), between1And2);
        partitionsToExpectedCut.put(initSet(1, 3), between1And3);
        partitionsToExpectedCut.put(initSet(1, 4), between1And4);
        partitionsToExpectedCut.put(initSet(2, 3), between2And3);
        partitionsToExpectedCut.put(initSet(2, 4), between2And4);
        partitionsToExpectedCut.put(initSet(3, 4), between3And4);
        ...
        for(int uid1 : uidToPidToFriendCount.keySet()) {
            ...
            for(int pid2 : uidToPidToFriendCount.get(uid1).keySet()) {
                ...
                int noMoveCut = partitionsToExpectedCut.get(initSet(pid1, pid2));
                int friendsOnP1 = Sets.intersection(partitions.get(pid1), bidirectionalFriendships.get(uid1)).size();
                int friendsOnP2 = Sets.intersection(partitions.get(pid2), bidirectionalFriendships.get(uid1)).size();
                int delta = friendsOnP1 - friendsOnP2;
                int result = calcEdgeCutMove(uid1, uid2, bidirectionalFriendships, partitions, uidToPidMap);
                assertTrue(result == (noMoveCut + delta));
            }
        }
    }

    @Test
    public void testGetCutBetweenPartitions() {
        Map<Integer, Set<Integer>> partitions = new HashMap<>();
        partitions.put(1, initSet( 1,  2,  3,  4,  5));
        partitions.put(2, initSet( 6,  7,  8,  9, 10));
        partitions.put(3, initSet(11, 12, 13, 14, 15));
        ...
        assertTrue(between1And2 == getCutBetweenPartitions(1, 2, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between1And2 == getCutBetweenPartitions(2, 1, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between1And3 == getCutBetweenPartitions(1, 3, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between1And3 == getCutBetweenPartitions(3, 1, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between1And4 == getCutBetweenPartitions(1, 4, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between1And4 == getCutBetweenPartitions(4, 1, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between2And3 == getCutBetweenPartitions(2, 3, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between2And3 == getCutBetweenPartitions(3, 2, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between2And4 == getCutBetweenPartitions(2, 4, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between2And4 == getCutBetweenPartitions(4, 2, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between3And4 == getCutBetweenPartitions(3, 4, bidirectionalFriendships, partitions, uidToPidMap));
        assertTrue(between3And4 == getCutBetweenPartitions(4, 3, bidirectionalFriendships, partitions, uidToPidMap));
